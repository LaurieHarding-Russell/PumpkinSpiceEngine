...
export class BufferFactory implements BufferFactoryInterface {
...
// BufferFactory: An interface for a factory class that takes in your ModelReferences and interleaves there data so that it can be stored together in one shader buffer.
// If keeps track of where each model is stored in the buffer.
export interface BufferFactoryInterface {
    modelReferences: Map<string, ModelReference>;

    addModel(name: string, modelInfo: ModelInfo, shader: ShadersType | string): BufferFactory;
    create(gl: WebGL2RenderingContext): Buffers;
}